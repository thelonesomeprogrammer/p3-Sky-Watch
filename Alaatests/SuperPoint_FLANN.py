...
def view_all_clusters(sat_kpts, drone_kpts, scores, satellite_image_path, plot=False):
    ...
    Args:
        ...
        satellite_image_path (str): Path to the satellite image.
    ...
    db = DBSCAN(eps=10, min_samples=5).fit(sat_kpts)
    labels = db.labels_
...
def filter_matches_with_ransac(kpts0, kpts1, matches, ransac_thresh=2.0):
    ...
    inlier_matches = [idx_pairs[i] for i, is_inlier in enumerate(inlier_mask) if is_inlier]
...
def match_features(feats0, feats1, k=2, ratio_thresh=0.75):
    ...
    Args:
        ...
        ratio_thresh (float, optional): Threshold for Lowe's ratio test. Defaults to 0.751
    ...
    # Create Flann object
    # FLANN_INDEX_KDTREE = 5  # KDTree
    # index_params = dict(algorithm=FLANN_INDEX_KDTREE, trees=1)  # Adjust number of trees for performance
    # search_params = dict(checks=100)  # Number of times the tree(s) in the index should be recursively traversed
    # flann = cv2.FlannBasedMatcher(index_params, search_params)

    # Perform KNN matching
    # knn_matches = flann.knnMatch(desc0, desc1, k=k)
    bf = cv2.BFMatcher(cv2.NORM_L2, crossCheck=False)
    knn_matches = bf.knnMatch(desc0, desc1, k=k)
...
def main():
    ...
    for drone_image_name, drone_feats in drone_features.items():
        ...
        for sat_image_name in satellite_image_names:
            ...
            all_matches = []
            # Initialize lists to collect all keypoints and scores
            all_satellite_kpts = []
            all_drone_kpts = []
            all_match_scores = []
                ...
                if matched_indices is not None:
                    ...
                    if num_valid > 0:
                        ...
                        if len(inlier_matches) > 0:
                            ...
                            # Collect satellite and drone keypoints
                            for idx_pair in inlier_matches:
                                drone_idx, sat_idx = idx_pair
                                drone_kpt = drone_feats['keypoints'][drone_idx]
                                satellite_kpt = tile_feats['keypoints'][sat_idx]
                                all_drone_kpts.append(drone_kpt.cpu().numpy())
                                all_satellite_kpts.append(satellite_kpt.cpu().numpy())
                                # Collect matching score (if available)
                                all_match_scores.append(1.0)  # Assign a default score or use actual scores if available

            # Convert collected keypoints and scores to numpy arrays
            all_drone_kpts = np.array(all_drone_kpts)
            all_satellite_kpts = np.array(all_satellite_kpts)
            ...
            for idx in range(num_tiles_to_visualize):
                ...
                # Adjust satellite_kpts to tile coordinates
                satellite_kpts_tile = satellite_kpts.clone()
                satellite_kpts_tile[:, 0] -= x
                satellite_kpts_tile[:, 1] -= y
            ...
            visualize_clusters(satellite_image_path, all_satellite_kpts, all_match_scores)
            view_all_clusters(all_satellite_kpts, all_drone_kpts, all_match_scores, satellite_image_path, plot=True)
