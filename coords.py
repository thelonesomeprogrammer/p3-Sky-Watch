...
def xy_to_coords(boundaries, sat_res, feature_coords):
    ...
    east_west = abs(boundaries[2] - west_bound)
    pix_lat_long_eq = [east_west / sat_res[0], north_south / sat_res[1]]
    loc_lat_long = []
    for i in range(0, 4):
        loc_lat_long.append([boundaries[0] - (abs(feature_coords[i][1] * pix_lat_long_eq[1])), (boundaries[3] + (abs(feature_coords[i][0] * pix_lat_long_eq[0])))])
    return loc_lat_long

def main(): ## test function
    fake_featues = [[2000.0,2000.0],[2010.0,2010.0],[2020.0,2010.0],[2034.0,2057.0]] ## fake feature matches 
    boundaries = load_csv_to_arr('./SatData/boundaries.txt') ## load bonds 
    ...
    loc_long_lat = xy_to_coords(boundaries, sat_res, x_y) ## locations of features
    print (boundaries)
    print(loc_long_lat)
    return
